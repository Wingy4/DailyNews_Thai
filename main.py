...
def build_user_message(english, thai, youtube, trends):
    en_lines = []
    ...
    offset = len(th_lines)
    for j, n in enumerate(youtube, 1):
        th_lines.append(f"{offset + j}. [YouTube: {n['origin']}, {n['tag']}] {n['title']}\n   ссылка: {n['link']}")
    ...
    return (
        ...
        f"=== ТАЙСКАЯ ЛЕНТА, ТЕКСТ И ВИДЕО (контроль охвата) ===\n{th_block}\n\n"
...
4. Затем раздел <b>Чего нет в англоязычных СМИ</b>: пройдись по тайской ленте и коротко назови сюжеты, которые есть там, но которых нет в англоязычной части. Это показывает, что внутренняя тайская повестка освещает, а международные издания пропускают. Если таких расхождений нет, честно напиши, что повестки совпали. Тайская лента теперь включает не только статьи, но и заголовки роликов тайских новостных телеканалов на ютубе, помеченные как [YouTube: канал, тег]. Используй их наравне с текстовыми новостями. Ролики с тегом «аналитика/обсуждение» считай более весомым сигналом того, что тема реально волнует общество, и такой сюжет стоит упомянуть заметнее.
...
def generate_digest(english, thai, youtube, trends):
    user_msg = build_user_message(english, thai, youtube, trends)
...
def main():
    ...
    digest = generate_digest(english, thai, youtube, trends)
